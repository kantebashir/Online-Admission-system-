...
from forms.applicant_form import ApplicantProfileForm, ApplicantPrevEducationFormSet
import json
...
class ApplicantProfileView(generic.ListView):
    ...
    def get_queryset(self):
        qs = ApplicantProfile.objects.select_related(
            'owner').filter(owner=self.request.user).only("owner__username", "fathers_name", "mothers_name", "Firs_Name", "student_pic")
        return qs
...
class CreateApplicantProfileView(SuccessMessageMixin, AictiveApplicantRequiredMixin, generic.CreateView):
    ...
    def get_context_data(self, **kwargs):
        context = super(CreateApplicantProfileView, self).get_context_data(**kwargs)
        if self.request.POST:
            context['preveducation'] = ApplicantPrevEducationFormSet( self.request.POST, self.request.FILES)
        else:
...
class EditApplicantProfileView(SuccessMessageMixin, AictiveApplicantRequiredMixin, generic.UpdateView):
    ...
    def get_context_data(self, **kwargs):
        context = super(EditApplicantProfileView, self).get_context_data(**kwargs)
        if self.request.POST:
            context['preveducation'] = ApplicantPrevEducationFormSet( self.request.POST, self.request.FILES, instance=self.object)
        else:
            query_set = ApplicantPrevEducation.objects.select_related('applicant').filter( applicant=self.object)
            print(query_set)
            context['preveducation'] = ApplicantPrevEducationFormSet(instance=self.object)
...
class ApplicationStatusView(AictiveApplicantRequiredMixin, generic.edit.DeleteView):
    def get(self, request, *args, **kwargs):
        ...
        application_obj = list( Application.objects.filter(applicant=applicant_obj))
...
class ApplicantAdmitCardView(AictiveApplicantRequiredMixin, generic.ListView):
    ...
    def get_queryset(self):
        qs = Application.objects.select_related('owner', 'applicant', 'institute', 'subject').filter( owner=self.request.user, status='1', paid=True).only("applicant__student_name","institute__institute_name","subject__subject_name","level","admit_card","owner__username")
        return qs
